...
* children: This prop will be passed to a component if you define it or if you include any JSX within the components opening and closing tags.  This can be used to make container components reusable by just swapping out the children they render.
* React.Children: Part of the top-level React API, you can use the Children methods to interface with a components children since props.children is array-like.  A few useful methods are size, only, and toArray.

#Reselect
* A library that can be used with React in order to create memoized functions that can be used to create derived data.  This can increase the performance of your React application by preventing unnecessary rerenders and only performing expensive calculations when the state has actually changed.
* defaultEqualityCheck: If you don't provide an equality check function to reselect, the default check will simply be ===.
...
* connect: The connect function is used to register your component with Redux.  You can optionally provide mapStateToProps and mapDispatchToProps in order to pull in specific pieces of state and dispatch functions.  Otherwise you will receive the Redux dispatch function as a prop when your component is connected.
